...
const AdminVotesView = ({ awardId }: AwardVotesViewProps) => {
            ...
            awardDataLoading ? (
              ...
              <>
                <div className="flex flex-row items-center gap-2">
                  ...
                  <Typography className="text-base text-amber-500 capitalize font-LatoBold">{awardData?.data.title} Votes</Typography>
                </div>
                <Button size="sm" onClick={handleOpenNominationsDialog} className="hidden bg-amber-300 text-gray-900 font-LatoBold hover:bg-transparent hover:border hover:border-gray-900 transition ease-linear">manage</Button>
              </>
            ) : ""
          }
        </div>
          ...
          <div>
            {
              nominationDataLoading ? <p className="text-base text-gray-900 font-LatoRegular py-3 text-center">Processing nomination status...</p> : nominationDataError ? <p className="text-base text-gray-700 font-LatoRegular text-center py-3 bg-gray-100">Award Nominationations not activated</p> : nominationData ? (
                ...
                <div className="flex flex-col gap-2">
                    {
                      nominationDataError ? (<p>A problem fetching votes data</p>) :
                        nominationData.data.length == 0 ? <p className="text-base text-gray-700 font-LatoRegular text-center py-3 bg-gray-100 capitalize mt-4">no votes found.</p>
                                    ...
                                    <div key={count} className="py-2 px-2 bg-transparent">
                                      ...
                                      <div className="flex flex-row items-center justify-between">
                                        <div className="flex flex-row items-center ">
                                          <GoDotFill />
                                          <NavLink to={`/admin/awards/${nav.awardId}/votes/${nomination.category_id}`} state={{genre_id: genreId, category_type_id: nomination.category_type_id}}>
