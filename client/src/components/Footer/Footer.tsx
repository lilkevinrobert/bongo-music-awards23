...
const Footer = () => {
  ...
  const renderLinks = () => {
    return LINKS.map((linkGroup, index) => (
      <div key={index}>
        <Typography variant="small" className="mb-3 font-LatoBold opacity-50 text-slate-900 uppercase">{linkGroup.title}</Typography>
        <ul>
          {linkGroup.items.map((item, itemIndex) => (
            <li key={itemIndex} className="mb-2">
              <a href={getHref(linkGroup.title, item)} className="text-sm text-slate-950 font-LatoBold capitalize opacity-80 transition ease-in-out hover:underline hover:underline-offset-4">
                {item}</a>
            </li>
          ))}
        </ul>
    ...
    <footer className="relative w-full bg-white">
      <div className="mx-auto w-full max-w-7xl py-8">
          ...
          <div className="bg-transprent flex flex-col items-start px-6 md:px-0 md:items-start justify-center">
            ...
            <div className="md:self-start font-LatoRegular text-shadow-md antialiased flex flex-col py-4 gap-2">
              <div className="flex items-center justify-start gap-4">
                <MdPhone className="text-2xl text-slate-950 opacity-50" />
                <Typography className="text-slate-950 opacity-70">
                  +255 714 456 680
                </Typography>
              </div>
              <div className="flex items-center justify-start gap-4">
                <HiMiniGlobeAlt className="text-2xl text-slate-950 opacity-50" />
                <Typography className="text-slate-950 opacity-70">
                  josia@bma.com
                </Typography>
              </div>
              <div className="flex items-center justify-start gap-4">
                <MdLocationPin className="text-2xl text-slate-950 opacity-50" />
                <Typography className="text-slate-950 opacity-70">
        ...
        <div className="mt-12 flex w-full flex-col items-center justify-center border-t border-blue-gray-50 py-4 md:flex-row md:justify-between">
          <Typography
            variant="small"
            className="mb-4 text-center font-LatoRegular text-slate-500 md:mb-0"
          >
            &copy; {currentYear}{" "}
