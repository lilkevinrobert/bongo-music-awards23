...
const Footer = () => {
  ...
  const renderLinks = () => {
    return LINKS.map((linkGroup, index) => (
      <div key={index}>
        <Typography variant="small" className="mb-3 font-LatoBold opacity-50 text-gray-950 uppercase">{linkGroup.title}</Typography>
        <ul>
          {linkGroup.items.map((item, itemIndex) => (
            <li key={itemIndex} className="mb-2">
              <a href={getHref(linkGroup.title, item)} className="text-sm text-gray-950 font-LatoBold capitalize transition ease-in-out hover:underline hover:underline-offset-4">
                {item}</a>
            </li>
          ))}
        </ul>
      ...
      <div className="mx-auto w-full max-w-7xl py-8">
        <div className="grid grid-cols-1 justify-between gap-4 md:grid-cols-2">
          <div className="bg-transprent flex flex-col items-start px-6 md:px-0 md:items-start justify-center">
            <img src={Logo} alt="bongo music awards logo" className="text-sm w-36 mb-6" />
